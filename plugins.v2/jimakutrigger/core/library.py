...
真正可靠的中文名→ID 来源是媒体库自己：MoviePilot 刮削时会在剧集根目录写 tvshow.nfo，
里面既有中文标题也有 tmdb_id。于是流程变成：
    浏览 /媒体 → 看到中文剧名 → 点一下 → 自动带出 tmdb_id 和文件夹路径 → 批量补扫
没有 nfo 的目录也照样列出来，只是 tmdb_id 需要人工填。
...
def read_nfo(folder: Path) -> tuple[int | None, str, str]:
    """从剧集目录的 nfo 里取 (tmdb_id, 标题, 年份)，取不到就返回空。

    MoviePilot/Jellyfin 刮削产物，常见两种写法：
        <uniqueid type="tmdb">209867</uniqueid>
        <tmdbid>209867</tmdbid>
    """
    for nfo in ("tvshow.nfo", "movie.nfo"):
        f = folder / nfo
        if not f.is_file():
            continue
        try:
        ...
        for el in root.findall("uniqueid"):
            if (el.get("type") or "").lower() in ("tmdb", "themoviedb") and (el.text or "").strip().isdigit():
                tmdb_id = int(el.text.strip())
                break
        if tmdb_id is None:
            for tag in ("tmdbid", "tmdb_id", "id"):
                el = root.find(tag)
...
def _entry_for(child: Path, tmdb_id, title, year, with_stats: bool) -> LibraryEntry:
    e = LibraryEntry(name=child.name, path=str(child), tmdb_id=tmdb_id, title=title, year=year)
    try:
        ...
        e.note = "无权限读取"
    if tmdb_id is None:
        e.note = e.note or "没找到 nfo，tmdb_id 需要手动填"
    return e
...
def find_shows(q: str = "", root: str = "") -> dict:
    ...
    matched = []
    for rp, child in all_shows:
        tmdb_id, title, year = read_nfo(child)
        if q in child.name.lower() or q in (title or "").lower():
            matched.append((child, tmdb_id, title, year))

    truncated = len(matched) > _RESULT_LIMIT
    shown = matched[:_RESULT_LIMIT]
    with_stats = len(shown) <= _STATS_BUDGET
    entries = [_entry_for(c, t, ti, y, with_stats) for c, t, ti, y in shown]
...
def browse(path: str, q: str = "", deep_stats: bool | None = None) -> dict:
    ...
    for child in children:
        tmdb_id, title, year = read_nfo(child)
        if q and q not in child.name.lower() and q not in (title or "").lower():
            continue
        prepared.append((child, tmdb_id, title, year))

    with_stats = deep_stats if deep_stats is not None else len(prepared) <= _STATS_BUDGET
    entries = [_entry_for(c, t, ti, y, with_stats) for c, t, ti, y in prepared]
    note = ""
    if not entries:
